...
if [ -e mingw-libv3270.tar.xz ]; then
	...
	cd ./.build/libv3270
	./autogen.sh > $LOGFILE 2>&1 || die "libv3270 Autogen failure"
	./configure > $LOGFILE 2>&1 || die "libv3270 Configure failure"
	make clean > $LOGFILE 2>&1 || die "libv3270 Make clean failure"
	make all  > $LOGFILE 2>&1 || die "libv3270 Make failure"
	make install  > $LOGFILE 2>&1 || die "libv3270 Install failure"
	cd ../..
fi
