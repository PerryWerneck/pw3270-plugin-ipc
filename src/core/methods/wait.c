...
int ipc3270_method_wait_for_connected(GObject *session, GVariant *request, GObject *response, GError G_GNUC_UNUSED(**error)) {
	guint seconds = 1;
	g_variant_get(request, "(u)", &seconds);
	debug("seconds=%u",seconds);
	ipc3270_response_append_int32(response,lib3270_wait_for_connected(ipc3270_get_session(session),seconds));
	return 0;
}
